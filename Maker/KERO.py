  ...
  for i in bots:
      ...
      try:
       if not i["bot_username"] in Done:
        ...
        logger = i["logger"]
        bot = Client("KERO", api_id=API_ID, api_hash=API_HASH, bot_token=TOKEN, in_memory=True, plugins=dict(root="KERO"))
        user = Client("KERO", api_id=API_ID, api_hash=API_HASH, session_string=SESSION, in_memory=True)
        await bot.start()
        await user.start()
